...
export const login = {
  ...
  resolve: async (_: any, args: any, ctx: Context): Promise<Result> => {
    return await UserService.login(ctx.prisma, args.email, args.password);
  },
  type: "loginResult",
};
...
export const checkEmail = {
  ...
  resolve: async (_: any, args: any, ctx: Context): Promise<Result> => {
    return await UserService.checkEmail(ctx.prisma, args.email);
  },
  type: "statusResult",
};
...
export const sendAuthEmail = {
  ...
  resolve: async (_: any, args: any, ctx: Context) => {
    const result = await sendAuthCode(ctx.prisma, args.email);
    return result;
  },
  type: "statusResult",
};
export const authenticateEmail = {
  ...
  resolve: async (_: any, args: any, ctx: Context) => {
    const result = await checkAuthCode(ctx.prisma, args.email, args.code);
    return result;
  },
  type: "statusResult",
};
...
export const createUser = {
  ...
  resolve: async (_: any, user: UserCreateArgs, ctx: Context) => {
    const new_user = await UserService.createUser(ctx.prisma, user);
    const new_portfolio = await PortfolioService.createPortfolio(
      ctx.prisma,
      new_user.id
    );
    return new_user;
...
export const deleteUser = {
  resolve: async (_: any, args: any, ctx: Context) => {
    const deleted_user = await UserService.deleteUser(ctx.prisma, ctx.userId);
    return deleted_user;
  },
  type: "User",
};
